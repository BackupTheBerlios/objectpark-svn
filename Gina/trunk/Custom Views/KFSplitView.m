...
- (void)mouseDown:(NSEvent *)theEvent
{
	...
	BOOL delegateHasSplitPosConstraintFunc = NO;
    ...
    delegateHasSplitPosConstraintFunc = [kfDelegate respondsToSelector: @selector(splitView:constrainSplitPosition:ofSubviewAt:)];
    ...
    do {
        ...
        if (delegateHasSplitPosConstraintFunc) {		
			dividerCoord = [kfDelegate splitView: self constrainSplitPosition: dividerCoord ofSubviewAt: divider];
        }
...
- (void)adjustSubviews
{
	...
	if ([[self positionAutosaveName] length])
	{
		[self kfSavePositionUsingAutosaveName:self];
	}
}
...
- (void)kfLayoutSubviewsUsingThicknesses:(unsigned *)subviewThicknesses
{
	...
	// Change by A. Katerbau (adjustSubviews didn't recalculate the divider rects):
	[self kfRecalculateDividerRects];
}
...
- (BOOL)hasCollapsedSubviews
{
	return [kfCollapsedSubviews count] != 0;
}
