...
int sqlite3BtreeOpen(
					 ...
					 int flags,              /* Options */
					 unsigned addMemory           /* additional memory to allocate */
){
	...
	p = sqliteMalloc(sizeof(Btree)+addMemory);
	if( !p ){
		return SQLITE_NOMEM;
	}
...
int sqlite3BtreeRollback(Btree *p){
	...
	if( rc!=SQLITE_OK ){
		...
		while( pBt->pCursor ){
			...
			if( db ){
				//dth sqlite3AbortOtherActiveVdbes(db, 0);
			}
		}
	}
#endif
...
int sqlite3BtreeCursor(
	...
	*ppCur = 0;
	...
	pCur = sqliteMalloc( sizeof(*pCur) + addMemory);
	if( pCur==0 ){
		rc = SQLITE_NOMEM;
