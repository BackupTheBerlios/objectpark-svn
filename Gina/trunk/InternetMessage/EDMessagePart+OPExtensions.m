...
/*" Returns the receivers content as a user presentable attributed string. Returns nil if not decodable. "*/
{
    ...
    if (contentCoderClass != nil) // found a content coder willing to decode self
    {
        ...
        @try {
            ...
            if (NSDebugEnabled) NSLog(@"[%@ %@] Exception while extracting contents as attributed string. (%@)", [self class], NSStringFromSelector(_cmd), [localException reason]);
...
{
#warning GPG support disabled
	if (NO) return NSLocalizedString(@"Could not be verified - GPG is not installed (see http://macgpg.sourceforge.net/)", @"Signature description");

	NSArray *signatures = [self signatures];
	GPGSignature *signature = [signatures lastObject];
	
	EDEntityFieldCoder *coder = (EDEntityFieldCoder *)[self decoderForHeaderFieldNamed:@"content-type"];
