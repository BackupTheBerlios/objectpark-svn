...
//        localEffectiveRange.length = length;
    ...
    if ([encodersAndRanges count] == 1)
    {
        ...
        encoderClass = NSClassFromString([encoderAndRange firstObject]);
        range = NSRangeFromString([encoderAndRange secondObject]);
...
- (void) generateMessageIdWithSuffix: (NSString*) aString
{
    ...
    [self setBody:[NSString stringWithFormat:@"<G3%@%@>", md5sum, aString] forHeaderField:@"Message-ID"];
