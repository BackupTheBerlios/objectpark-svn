//---------------------------------------------------------------------------------------
//  OPTCPSocket.m created by erik
//  @(#)$Id: OPTCPSocket.m,v 1.3 2004/05/24 20:18:59 theisen Exp $
//
...
//---------------------------------------------------------------------------------------
    @implementation OPTCPSocket
//---------------------------------------------------------------------------------------

/*" This class implementes TCP sockets and some methods specific to such sockets. Most of the commonly used functionality is in its superclasses #OPIPSocket and #NSFileHandle as well as in the NSFileHandle category defined in this framework.
...
- (void)startListening
{
    if(listen(OPSOCKETHANDLE, 5) == -1)   // backlog length of 5 is maximum
        [NSException raise:NSFileHandleOperationException format:@"Unable start listening on local port: %s", strerror(OP_ERRNO)];
    flags.listening = YES;
}
