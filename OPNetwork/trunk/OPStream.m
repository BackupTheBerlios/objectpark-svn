//---------------------------------------------------------------------------------------
//  OPStream.m created by erik
//  @(#)$Id: OPStream.m,v 1.2 2005/06/06 23:49:06 theisen Exp $
//
...
/*" This class is meant to be used with sockets but it can work with all kinds of NSFileHandle. The main concept in OPStream is the conversion from raw data, chopped up into chunks of arbitrary size and delivered as NSData objects, into a stream of NSString objects representing meaningful units, lines for example. OPStream also contains functionality to simplify the implementation of common text-based internet protocols such as SMTP and NNTP. "*/
...
/*" Creates and returns a stream for an OPTCPSocket connected to port on host. "*/

+ (id)streamConnectedToHost:(NSHost *)host port:(unsigned short)port
{
    OPTCPSocket *socket;

    socket = [OPTCPSocket socket];
    [socket connectToHost:host port:port];
    return [self streamWithFileHandle:socket];
}


/*" Creates and returns a stream for an OPTCPSocket connected to port on the host with the name hostname. "*/
...
/*" Creates and returns a stream for an OPTCPSocket connected to port on host. %SendTimeout and %receiveTimout are set on this socket. "*/
...
/*" Creates and returns a stream for an OPTCPSocket connected to port on the host with the name hostname. %SendTimeout and %receiveTimout are set on this socket. "*/
...
/*
 * Returns a text, if available and nil otherwise.
 */
- (NSData*) availableTextData 
{
    BOOL           cont       = YES;
    NSMutableData* textBuffer = [NSMutableData dataWithCapacity: 400];
    NSAssert([self escapesLeadingDots],
             @"-availableTextData only works with the escapesLeadingDots option. Use availableData otherwise.");
    
    while (cont) {
        NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
        NSData* lineData = [self availableLineData];
        
        if (cont = (lineData!=nil)) {
            unsigned lineLength = [lineData length];
            if (lineLength>0) {
                const char* lineBytes = [lineData bytes];
                if (*lineBytes=='.') {
                    //MPWDebugLog(@"Found line starting with a dot and length is %d.", lineLength);
                    if (lineLength==1) {
                        [pool release];
                        // We found the end of text marker:
                        return textBuffer;
                    }
                    // handle dot (un)quoting:
                    // skip leading dot:
                    [textBuffer appendBytes: lineBytes+1 length: lineLength-1];
                } else {
                    [textBuffer appendBytes: lineBytes length: lineLength];
                }
            }
            [textBuffer appendData: linebreakSequences[flags.linebreakStyle]];
        }         
        [pool release];
    }
    // text not complete:
...
{
    //OPDebugLog2(STREAMDEBUG, OPINFO, @"%@ --> '%@'.", [self fileHandle], line);
